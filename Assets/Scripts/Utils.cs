...
public static class Utils {
  ...
  public static Rect GetScreenRect(Vector3 screenPosition1, Vector3 screenPosition2) {
    // Move origin from bottom to top left
  ...
  public static Rect GetBoundingBoxOnScreen(Bounds bounds, Camera camera) {
    ...
    for (int i = 0; i < vertices.Length; i++) {
      ...
      if (v.x < retVal.xMin) { retVal.xMin = v.x; }
      if (v.y < retVal.yMin) { retVal.yMin = v.y; }
      if (v.x > retVal.xMax) { retVal.xMax = v.x; }
      if (v.y > retVal.yMax) { retVal.yMax = v.y; }
    }
    return retVal;
  }
